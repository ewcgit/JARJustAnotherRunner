...
if((other.object_index != obj_grapes) && (playerHP = 1)){ // If an object that isn't grapes is touched, then game over.
...
else if((other.object_index != obj_grapes) && (playerHP > 1)){
	...
	global.points += 900; // 1000 points when a grapes item is eaten.
	audio_play_sound(apple, 100, false); // Grapes eaten sound plays and it doesn't loop.
	instance_destroy(obj_grapes); // Grapes removed from the screen after being consumed.
}